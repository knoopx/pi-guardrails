...
describe("tokenizeSql", () => {
  ...
  describe("simple words", () => {
    ...
    it("tokenizes with keywords", () => {
      expect(tokenizeSql("SELECT name FROM users WHERE active = 1")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "name"),
          token("word", "FROM"),
  ...
  describe("single-quoted strings", () => {
    it("parses a single-quoted string", () => {
      expect(tokenizeSql("SELECT 'hello' FROM users")).toEqual([
        [
          token("word", "SELECT"),
          token("string", "hello"),
          token("word", "FROM"),
    ...
    it("handles escape sequences in single quotes", () => {
      ...
      expect(tokenizeSql("SELECT 'it''s' FROM users")).toEqual([
        [
          token("word", "SELECT"),
          token("string", "it"),
          token("string", "s"),
    ...
    it("handles backslash at end of single-quoted string", () => {
      ...
      expect(tokenizeSql("SELECT 'path\\' FROM users")).toEqual([
        [token("word", "SELECT"), token("string", "path\\' FROM users")],
      ]);
    });
  });
  ...
  describe("double-quoted identifiers", () => {
    it("parses a double-quoted identifier", () => {
      expect(tokenizeSql('SELECT "user_name" FROM users')).toEqual([
        [
          token("word", "SELECT"),
    ...
    it("parses double-quoted string with special chars", () => {
      expect(tokenizeSql('SELECT "table-name" FROM users')).toEqual([
        [
          token("word", "SELECT"),
  ...
  describe("raw strings", () => {
    it("parses r'' raw string", () => {
      expect(tokenizeSql("SELECT r'hello world' FROM users")).toEqual([
        [
          token("word", "SELECT"),
    ...
    it("raw strings only parsed when preceding empty token position", () => {
      ...
      expect(tokenizeSql("SELECT abc r'hello'")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "abc"),
  ...
  describe("operators", () => {
    it("tokenizes opening parenthesis", () => {
      expect(tokenizeSql("SELECT count(*) FROM users")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "count"),
          token("operator", "("),
    ...
    it("tokenizes comma operator", () => {
      expect(tokenizeSql("SELECT a, b, c FROM t")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "a"),
          token("operator", ","),
    ...
    it("tokenizes semicolon", () => {
      expect(tokenizeSql("SELECT 1; SELECT 2")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "1"),
          token("operator", ";"),
    ...
    it("tokenizes colon and equals", () => {
      ...
      expect(tokenizeSql("SELECT :name, @var")).toEqual([
        [
          token("word", "SELECT"),
          token("operator", ":"),
          token("word", "name"),
  ...
  describe("word characters", () => {
    it("handles dots in identifiers", () => {
      expect(tokenizeSql("SELECT table.column FROM schema.table")).toEqual([
        [
          token("word", "SELECT"),
    ...
    it("handles underscores and digits", () => {
      expect(tokenizeSql("SELECT user_name1 FROM t2")).toEqual([
        [
          token("word", "SELECT"),
  ...
  describe("function call patterns", () => {
    it("tokenizes function call with arguments", () => {
      expect(tokenizeSql("SELECT read_csv_auto('file.csv')")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "read_csv_auto"),
          token("operator", "("),
    ...
    it("tokenizes function with multiple args", () => {
      expect(tokenizeSql("SELECT substr('hello', 0, 5)")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "substr"),
          token("operator", "("),
  ...
  describe("tilde and special paths", () => {
    it("tokenizes tilde as a word character", () => {
      expect(tokenizeSql("SELECT * FROM read_csv_auto('~/data.csv')")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "*"),
          token("word", "FROM"),
  ...
  describe("guardrail: read_csv_auto", () => {
    ...
    it("tokenizes read_csv_auto with options", () => {
      expect(
        tokenizeSql(
          "SELECT * FROM read_csv_auto('file.csv', header=true, auto_detect=true)",
        ),
      ).toEqual([
        [
          token("word", "SELECT"),
  ...
  describe("guardrail: AUTO_DETECT ON", () => {
    it("tokenizes SET AUTO_DETECT ON", () => {
      expect(tokenizeSql("SET AUTO_DETECT ON")).toEqual([
        [
          token("word", "SET"),
          token("word", "AUTO_DETECT"),
          token("word", "ON"),
  ...
  describe("guardrail: regexp_extract with r'' raw string", () => {
    it("tokenizes regexp_extract with r''", () => {
      expect(tokenizeSql("SELECT regexp_extract(col, r'pattern', 1)")).toEqual([
        [
          token("word", "SELECT"),
  ...
  describe("guardrail: string_to_split_to_array", () => {
    it("tokenizes string_to_split_to_array", () => {
      expect(
        tokenizeSql("SELECT string_to_split_to_array(str, delimiter)"),
      ).toEqual([
        [
          token("word", "SELECT"),
  ...
  describe("guardrail: list_agg", () => {
    it("tokenizes list_agg", () => {
      expect(tokenizeSql("SELECT list_agg(col)")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "list_agg"),
          token("operator", "("),
  ...
  describe("guardrail: string_agg", () => {
    it("tokenizes string_agg", () => {
      expect(tokenizeSql("SELECT string_agg(col, ',')")).toEqual([
        [
          token("word", "SELECT"),
  ...
  describe("guardrail: initcap", () => {
    it("tokenizes initcap", () => {
      expect(tokenizeSql("SELECT initcap(col)")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "initcap"),
          token("operator", "("),
  ...
  describe("guardrail: COPY TO", () => {
    it("tokenizes COPY TO", () => {
      expect(tokenizeSql("COPY t TO 'file.csv' (HEADER TRUE)")).toEqual([
        [
          token("word", "COPY"),
          token("word", "t"),
          token("word", "TO"),
  ...
  describe("guardrail: INSERT INTO read_csv_auto", () => {
    it("tokenizes INSERT INTO read_csv_auto", () => {
      expect(tokenizeSql("INSERT INTO read_csv_auto VALUES (1)")).toEqual([
        [
          token("word", "INSERT"),
          token("word", "INTO"),
          token("word", "read_csv_auto"),
  ...
  describe("guardrail: UPDATE read_csv_auto", () => {
    it("tokenizes UPDATE read_csv_auto", () => {
      expect(tokenizeSql("UPDATE read_csv_auto SET col = 1")).toEqual([
        [
          token("word", "UPDATE"),
          token("word", "read_csv_auto"),
          token("word", "SET"),
  ...
  describe("guardrail: LOAD csv FROM", () => {
    it("tokenizes LOAD csv FROM", () => {
      expect(tokenizeSql("LOAD csv FROM 'file.csv'")).toEqual([
        [
          token("word", "LOAD"),
          token("word", "csv"),
          token("word", "FROM"),
  ...
  describe("guardrail: LIKE ANY", () => {
    it("tokenizes LIKE ANY", () => {
      expect(
        tokenizeSql("SELECT * FROM t WHERE col LIKE ANY (ARRAY['a', 'b'])"),
      ).toEqual([
        [
          token("word", "SELECT"),
          token("word", "*"),
  ...
  describe("guardrail: duckdb -c subprocess", () => {
    it("tokenizes duckdb -c", () => {
      expect(tokenizeSql("duckdb -c 'SELECT 1'")).toEqual([
        [
          token("word", "duckdb"),
          token("word", "-c"),
  ...
  describe("guardrail: duckdb -csv subprocess", () => {
    it("tokenizes duckdb -csv", () => {
      expect(tokenizeSql("duckdb -c 'SELECT 1' | csv")).toEqual([
        [
          token("word", "duckdb"),
          token("word", "-c"),
          token("string", "SELECT 1"),
  ...
  describe("guardrail: WHERE OVER", () => {
    it("tokenizes WHERE with OVER", () => {
      expect(
        tokenizeSql("SELECT * FROM t WHERE rank() OVER (ORDER BY col) = 1"),
      ).toEqual([
        [
          token("word", "SELECT"),
          token("word", "*"),
  ...
  describe("guardrail: substr + unnest", () => {
    it("tokenizes substr on unnest result", () => {
      expect(tokenizeSql("SELECT substr(unnest(arr), 1, 3)")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "substr"),
          token("operator", "("),
  ...
  describe("guardrail: nested UNNEST", () => {
    it("tokenizes nested UNNEST calls", () => {
      expect(tokenizeSql("SELECT unnest(unnest(arr))")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "unnest"),
          token("operator", "("),
  ...
  describe("guardrail: CASE THEN 0", () => {
    it("tokenizes CASE THEN 0", () => {
      expect(tokenizeSql("SELECT CASE WHEN x THEN 0 ELSE 'str' END")).toEqual([
        [
          token("word", "SELECT"),
          token("word", "CASE"),
          token("word", "WHEN"),
