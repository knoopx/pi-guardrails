...
describe("primitive matchers", () => {
  ...
  describe("regex", () => {
    ...
    it("matches guardrail patterns: Bun.* APIs", () => {
      expect(regex(/Bun\.(file|write|spawn|inspect|env|\$)/).match(t(["Bun.file"]))).toBe(true);
      expect(regex(/Bun\.(file|write|spawn|inspect|env|\$)/).match(t(["Bun.spawn"]))).toBe(true);
      expect(regex(/Bun\.(file|write|spawn|inspect|env|\$)/).match(t(["console.log"]))).toBe(false);
    });
    ...
    it("matches guardrail patterns: github search", () => {
      expect(regex(/^https?:\/\/github\.com\/search/).match(t(["https://github.com/search?q=test"]))).toBe(true);
      expect(regex(/^https?:\/\/github\.com\/search/).match(t(["https://github.com/user/repo"]))).toBe(false);
    });
  });
...
describe("combinators", () => {
  ...
  describe("repeat", () => {
    ...
    it("matches in sequence", () => {
      const m = seq(word("find"), repeat(anyOf(word("-name"), word("-type"), word("."))), word("-name"));
      expect(m.match(t(["find",".","-name"]))).toBe(true);
    });
  });
  ...
  describe("repeat1", () => {
    ...
    it("matches in sequence with prefix", () => {
      const m = seq(word("find"), repeat1(anyOf(word("-name"), word("-type"))), word("."));
  ...
  describe("contains", () => {
    ...
    it("matches multi-token target", () => {
      ...
      expect(m.match(t(["SELECT","*","FROM","read_csv_auto","('file')"]))).toBe(true);
...
describe("integration: read_csv_auto with ~ path", () => {
  ...
  it("matches read_csv_auto with ~ path", () => {
    expect(m.match(t(["SELECT","*","FROM","read_csv_auto","('~/path.csv')"]))).toBe(true);
  });

  it("matches read_csv_auto with full path", () => {
    expect(m.match(t(["SELECT","*","FROM","read_csv_auto","('file.csv')"]))).toBe(true);
  });

  it("matches ~ anywhere in tokens", () => {
    expect(regex(/~/).match(t(["SELECT","*","FROM","read_csv_auto","('~/path.csv')"]))).toBe(true);
  });

  it("does NOT match word('.') as tilde", () => {
    // word("~") only matches the exact token "~", not ".pi/records"
    expect(contains(word("~")).match(t(["SELECT","*","FROM","read_csv_auto","('/home/user/.pi/records.csv')"]))).toBe(false);
  });
});
